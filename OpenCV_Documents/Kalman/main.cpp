...
int main(){
    ...
    /*
        ...
        type -> Type of the created matrices that should be CV_32F or CV_64F default = CV_32F
    */
    KalmanFilter KF(2, 1, 0); // (dynamParams, measureparams, controlParams)
    ...
    for(;;){
        ...
        // process Noise Convariance Matrix Q
        ...
        for(;;){
            ...
            double improveAngle = KF.statePost.at<float>(0);
            Point improvedPt = calcPoint(center, R, improveAngle);
            ...
            drawMarker(img, statePt, Scalar(255, 255, 255), cv::MARKER_SQUARE, 10, 1);

            // forecast one step
            Mat test = Mat(KF.transitionMatrix * KF.statePost);
            drawMarker(img, calcPoint(center, R, Mat(KF.transitionMatrix * KF.statePost).at<float>(0)), Scalar(255, 255, 0), cv::MARKER_SQUARE, 12, 1);
