...
static int getdir(const string _filename, vector<String> &files){
    ...
    if(!myfile.is_open()){
        cout << "CANNOT to read file";
        exit(0);
    } else {;
        size_t found = _filename.find_last_of("/\\");
        string line_str, path_to_file = _filename.substr(0, found);
        while(getline(myfile, line_str)){
            cout << path_to_file + string("/") + line_str << "\n";
...
int main(int argc, char* argv[]){
    ...
    reconstruct(image_paths, Rs_est, ts_est, K, points3d_estimated, is_projective);
    ...
    // recover estimated point3d
    vector<Vec3f> point_cloud_est;
    for(int i=0; i<points3d_estimated.size(); ++i){
        point_cloud_est.push_back(Vec3f(points3d_estimated[i]));
        cout << Vec3f(points3d_estimated[i]) << "\n";
    }
