...
int main(int argc, char** argv){
    ...
    String left_image = parser.get<String>(0);
    String right_image = parser.get<String>(1);
    String GT_image = parser.get<String>("GT");
    ...
    Mat left = imread(left_image, IMREAD_COLOR);
    if(left.empty()){
        cout << "CANNOT OPEN IMAGE FILE" << left_image;
        return -1;
    }

    Mat right = imread(right_image, IMREAD_COLOR);
    if(right.empty()){
        cout << "CANNOT OPEN IMAGE FILE" << right_image;
        return -1;
    }

    bool noGT;  
    Mat GT_disp;
    if(GT_image == "../data/aloeGT.png" && left_image != "../data/aloeL.jpg"){
        noGT = true;
    } else {
        noGT = false;
        if(readGT(GT_image, GT_disp) != 0){
            cout << "CANNOT read ground truth image file:" << GT_image;
            return -1;
        }
    }
    ...
    if(max_disp <= 0 || max_disp % 16 != 0){
        cout << "Incorrect amx_dispartiy value : it should be positive and divisible by 16";
        return -1;
    }
    ...
    if(filter == "wls_conf"){
        ...
        if(algo == "bm"){
            ...
            cvtColor(left_for_matcher, left_for_matcher, COLOR_BGRA2GRAY);
            cvtColor(right_for_matcher, right_for_matcher, COLOR_BGRA2GRAY);
